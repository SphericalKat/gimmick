...
class GithubRepository @Inject constructor(
    ...
    private suspend fun fetchEvents(token: String, username: String) {
        dataSource.getReceivedEvents(token, username).asFlow().flatMapMerge {
            flow {
                try { // try getting repo
                    ...
                    Log.e(
                        "REPOSITORY",
                        "Error while fetching repos for events:",
                        e
                    )
                }
                emit(it) // emit event
        ...
        }.collect { eventDao.insertEvent(it) }
    }
